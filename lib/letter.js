...
//				   console.log("Paused at " + symNode.nodeIdentifier())
	    ...
	    if ((sym instanceof lw.NontermReference && sym.isAutoExpandable)
		|| (sym instanceof lw.NontermExpansion && symNode.isLocallyControlledByComputer())) {
...
//		console.log("Newly-enabled option expanded at " + this.nodeIdentifier())
	...
	var node = this
	while (true) {
	    if (!(node.symbol instanceof lw.NontermParent))
	...
	if (this.symbol instanceof lw.NontermParent) {
	    ...
	    msgNode = { name: this.symbol.nonterminal.id,
			type: "nonterm",
			child: [ { type: "preamble",
				   child: preambleNode.child.map (treeJSON) },
				 { type: "expansion" } ] }
