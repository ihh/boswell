...
//	    console.log ("Adding moveListener[" + type + "] on " + channel)
	...
	if (!grammar.isSinglePlayer()) {
		...
		addControl ({ property: "role",
			      ...
			      inputDiv: $('<select/>').attr('name','nontermRole'+(++nteDummyCount)).append('<option value="=">Same role</option><option value="+">Next role</option><option value="-">Previous role</option>' + role.map(function(r){return'<option value="'+r+'">Role #'+r+'</option>'}).join('')),
			      hasWarning: false,
			      getInput: function(){return this.inputDiv.val()},
			      setInput: function(x){this.inputDiv.val(x)},
			      tooltip: "This control selects which role (i.e. which human or computer player in a multiplayer game) has control of this choice. As well as specifying roles directly, you can specify that the controller is the same as for the parent choice ('Same role'), or the previous player, or the next player.",
	...
	var preambleInputDiv = $('<textarea rows="3"/>')
