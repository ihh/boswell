...
//	var editorArgs = {tooltips:false}
 ...
 return $("#playerName")[0].value }
	...
	function setPreamble(newPreamble) { nonterm.preamble = newPreamble; preambleInputDiv.val (nonterm.preambleAsText()) }
	function getPreambleWarning() { return nontermEditor.preambleControl.warningDiv }
	var preambleInputChange = makeRhsInputChange (grammarEditor, getPreamble, setPreamble, function(){}, getPreambleWarning)
...
//			 filter: validatePlainText(function(){return nontermEditor.preambleControl.warningDiv},"The preamble is not expanded or preprocessed at all, and so"),
    ...
    function makeRhsInputChange (grammarEditor, getRhs, setRhs, warnEmptyRhs, getRhsWarningDiv) {
	var grammar = grammarEditor.grammar
	var notify = grammarEditor.notifyChange
	return function rhsInputChange(e) {
	    var oldRhs = getRhs();
...
//	setLocation()
	...
	var rhsInputChange = makeRhsInputChange (grammarEditor, getRhs, setRhs, warnEmptyRhs, getRhsInputWarning)
...
//			console.log("delayedExpand of " + symNode.nodeIdentifier())
			var rule = sym.randomRule (letter, symNode.getScope());
...
//		span.appendChild (select);
    ...
    lw.NontermReference.prototype.randomRule = function(letter,scope) {
	return this.nonterminal.randomRule (letter, scope)
    }
    ...
    lw.NontermParent.prototype.randomRule = function() {
	...
	startParent.randomRule().expand (this.root.span, this.root.placeholderSpan, this.root)
	...
	while (match = nontermRe.exec(text)) {
		...
		if (nonterm.random) {
		    var rule = nonterm.randomRule(letter,scope)
