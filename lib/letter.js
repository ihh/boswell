...
//	setLocation()
	...
	function validateWeight() {
	    var hintWarningDiv = ruleEditor.weightExprInputWarning
	    if (typeof random == 'undefined') random = nonterm.random;
	    if (random)
		return validateAsWeight ("weightExpr")
	    else  // !random
		validatePlainText(function(){return hintWarningDiv},"For choices whose expansion is chosen by the player, the hint will not itself be expanded, and so")(rule.hint)
	}
...
//				 console.log(s.nodeIdentifier() + "  decrementing unexpandedChildren count of  " + p.nodeIdentifier() + "  (currently " + p.unexpandedChildren + ")")
	...
	if (this.hasActiveRules (letter)) {
		...
		activeRules = activeRuleIndex.sort(function(a,b){
		    return activeRuleWeight[a] - activeRuleWeight[b]
		}).map(function(i){
...
//		span.appendChild (select);
    ...
    lw.NontermReference.prototype.expandText = function(letter,scope,text) {
	// expand computer-controlled local nonterminals and parameter assignments
	...
	while (match = nontermRe.exec(text)) {
	    ...
	    if ((nontermId in letter.grammar) && !(nontermId in visited)) {
		visited[nontermId] = 1
		var nonterm = letter.grammar.nonterm[nontermId]
		if (nonterm.random && (!("role" in nonterm) || nonterm.role == "=")) {
	...
	return this.expandText (letter, scope, this.placeholder())
    }

    lw.NontermReference.prototype.expandedPrompt = function(letter,scope) {
	return this.expandText (letter, scope, this.prompt())
    }
