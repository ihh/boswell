...
//	    console.log ("Adding moveListener[" + type + "] on " + channel)
	...
	if (gotPreamble || gotPlaceholder || gotPrompt) {
	    str += "["
		+ (gotPreamble
		   ? (rhsToString(props.preamble,normalize,newId)
	...
	addProperty (this, "Symbol @", "id", {type:"text",size:15,keepFocus:false,hasWarning:false,parentDiv:propColumn[c++]}, "The word used to identify this choice.", grammarEditor.nontermRenamingFilter(nonterm))
	...
	if (!grammar.isSinglePlayer()) {
		...
		addControl ({ property: "role",
			      ...
			      tooltip: "This control selects which role (i.e. which human or computer player in a multiplayer game) has control of this choice. As well as specifying roles directly, you can specify that the controller is the same as for the parent symbol, or the previous player, or the next player.",
	...
	this.preambleControl =
	    addControl ({property: "preamble",
			 ...
			 tooltip: "This text is called the preamble. It is displayed wherever the choice, " + nid + ", occurs in the narrative, and it goes before any other text or controllers associated with " + nid + ". It is not removed or changed after the player makes a choice, although it may be hidden by one-way choices."})
...
//	setLocation()
	...
	addRuleControl ({property: "weightExpr",
			 name: "Weight ",
			 tooltip: "This is the probability that the hint will be available.",
	...
	for (var rhsPos = 0; rhsPos < this.rhs.length; ++rhsPos) {
	    ...
	    // which will then be automagically expanded to NontermPreamble and NontermExpansion
...
//		span.appendChild (select);
	...
	if (typeof(node) != 'undefined' && node.atMaxRecursionDepth())
	    return emptyRule
	var rhs = [new lw.NontermPreamble(this), new lw.NontermExpansion(this)]
	...
	startParent
	    .randomRule (this, null, this.root)  // generates @start.preamble@start.expansion
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    // there are three NontermReference nodes: NontermParent, NontermPreamble, NontermExpansion
	    // we only attach placeholder + controller/listener to the latter
...
//		console.log("Newly-enabled option expanded at " + this.nodeIdentifier())
	...
	var node = this
	while (true) {
	    if (!(node.symbol instanceof lw.NontermPreamble || node.symbol instanceof lw.NontermExpansion))
		...
		suffices.unshift (node.symbol instanceof lw.NontermPreamble ? 'p' : 'x')
	    node = parent
	}
	...
	if (this.symbol instanceof lw.NontermParent) {
		...
		var preambleNode = this.child[0], expansionNode = this.child[1]
