 ...
 return $("#playerName")[0].value }
	...
	function makeRhsExpr(props,rules) {
		...
		if (rule.weightExpr != null)
		    str += rule.weightExpr + (hint_with_modifiers.length ? ("=>" + hint_with_modifiers) : "") + "=>"
...
//		span.appendChild (select);
    ...
    // calculate role
    lw.Nonterm.prototype.computeRole = function(grammar,parentRole) {
	var symbolRole = this.role, totalRoles = grammar.roles
	var role = null
	switch (symbolRole) {
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    if (parentNode instanceof LetterWriter.Node)
		this.role = symbol.nonterminal.computeRole (letter.grammar, parentNode.role)
	    else
