...
//				 console.log(s.nodeIdentifier() + "  decrementing unexpandedChildren count of  " + p.nodeIdentifier() + "  (currently " + p.unexpandedChildren + ")")
    ...
    lw.Scope = function(parent,role) { extend (this, { parent:parent, paramValue:{}, role: role, complete: false }) }
...
//		console.log("Newly-enabled option expanded at " + this.nodeIdentifier())
	...
	var scope = node.hasOlderSibling()
	    ? node.previousSibling().getScope(true)
	    : (node.hasParent()
	       ? node.parent.getScope(false)
	       : new lw.Scope())

// scope.startNewScopeAndThenUpdate(node):
//		    var inner = new lw.Scope (scope, node.role)
//		    inner.role = node.role
//		    node.symbol.updateScope(inner,args)

// scope.efficientlyCloneScopeAtPointOfDivergenceAndThenUpdate(node):
...
// node.isUnexpandedNontermReference():
//                  return node.symbol instanceof lw.NontermReference && !("expansionRule" in node)

	if (includingSelf)
	    scope = node.symbol instanceof lw.ParamAssignment
		? (node.isOldestSiblingWithLocalParamAssignment()
		   ? scope.startNewScopeAndThenUpdate(node)  // sets newScope.role = node.role
		   : scope.efficientlyCloneScopeAtPointOfDivergenceAndThenUpdate(node))
	    : (node.hasChildren()
	       ? node.lastChild().getScope(true)
	       : scope)

	if (node.isUnexpandedNontermReference())
	{ scope = scope.clone(); scope.complete = false }
...
//    ? (this.isOldestSiblingWithLocalParamAssignment()
//       ? s.startNewScopeAndThenUpdate()
...
//     { s2 = s2.clone(); s2.complete = false }
	...
	var finalScope = new lw.Scope()
	var root = node.letter.root
	if (root instanceof lw.Node) {
	    finalScope.role = root.role
//	    finalScope.complete = true

	    function updateScope(node,last,scope,args) {
		if (node === last) {
		    finalScope = scope
		    if (!includingSelf)
			return
		}
		if (node.symbol instanceof lw.NontermReference) {
		    var inner = new lw.Scope (scope, node.role)
		    inner.role = node.role
		    node.iterateChild (function() { updateScope(this,last,inner,args) }, last)

//		    if (...scope is not complete/final...) {
//			var s = scope
//			while (s != null) { s.complete = false; s = s.parent }
//		    }

		} else if (node.symbol instanceof lw.ParamAssignment)
		    node.symbol.updateScope(scope,args)
	    }

	    updateScope (root, this, finalScope, {letter:node.letter})
	}
	finalScope.role = this.role
//	if (finalScope.complete)
//	    node[propName] = finalScope
	return finalScope
    }
