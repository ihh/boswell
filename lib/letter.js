var LetterWriter = (function(){
    ...
    var anonymousNonterminalPrefix = "x";
    var newParamPrefix = "Attribute";
    lw.defaultNever = "0";
    lw.defaultAlways = "100";

    var wordForNonterminal = "symbol"
    var nontermAnchorPrefix = wordForNonterminal + "_"
    ...
    function plural(n,noun)  // or plural(noun)
    { if (typeof(noun) == 'undefined') { noun = n; n = '' }  // convert plural(noun) to plural('',noun)
      return (n=='' ? '' : (n + " ")) + noun + (n == 1 ? "" : "s") }
    function capitalize(noun) { return noun.charAt(0).toUpperCase() + noun.substring(1) }
	...
	var editorArgs = { tooltips: true, alwaysShowParseTree: false, startWithIDEVisible: false }
	for (var prop in editorArgs) {
	    var propKey = storageKey + '.' + prop
	    nav['store_' + prop] = (function(prop,key) {
		return function(val) {
		    editorArgs[prop] = val
		    if (editor != null)
			editor[prop] = val
	    ...
	    var storedVal = localStorage.getItem (propKey)
	    if (storedVal != null)
		editorArgs[prop] = eval (storedVal)
	}
	...
	nav.changeShowIDE = function() {
	    if (showIDEChecked()) {
		$("#editor,#editorMapContainer,#editorDivider,.letterDebuggerShortcuts").show()
		$("#letterWriterBackground").addClass("letterWriterBackgroundEditMode").removeClass("letterWriterBackgroundPlayMode")
		nav.showDebugParseTree()
		nav.showDebugMap()
	    } else {
		$("#editor,#editorMapContainer,#editorDivider,.letterDebuggerShortcuts").hide()
		$("#letterWriterBackground").removeClass("letterWriterBackgroundEditMode").addClass("letterWriterBackgroundPlayMode")

		if (editor == null || !editor.alwaysShowParseTree)
		    nav.hideDebugParseTree()
	    }
	}

	nav.changeShowTooltips = function() {
	    nav.store_tooltips ($("#showTooltipsCheckbox")[0].checked ? true : false)
	    nav.createEditor()
	}

	nav.changeAlwaysShowParseTree = function() {
	    nav.store_alwaysShowParseTree ($("#alwaysShowParseTreeCheckbox")[0].checked ? true : false)
	}

	nav.changeStartWithIDEVisible = function() {
	    nav.store_startWithIDEVisible ($("#startWithIDEVisibleCheckbox")[0].checked ? true : false)
	}
	...
	nav.createEditor = function() {
	    ...
	    $("#editorNontermListHeader").text(capitalize(plural(wordForNonterminal)))
	    $("#showTooltipsCheckbox").prop("checked",editor.tooltips)
	    $("#startWithIDEVisibleCheckbox").prop("checked",editor.startWithIDEVisible)
	    $("#alwaysShowParseTreeCheckbox").prop("checked",editor.alwaysShowParseTree)
	}
	...
	$("#redoEdit").click(nav.redoEdit)

	// debugger
	nav.showDebugParseTree = function() {
	    $("#gameContainer").attr("style","border-style:solid")
	    $("#scoreContainer").attr("style","position:static;width:auto;bottom:auto")
	    $("#editorParseTreeContainer").show()
	    letter.showDebugParseTree()
	}

	nav.hideDebugParseTree = function() {
	    $("#gameContainer").attr("style","border-style:none;left:20%")
	    $("#scoreContainer").attr("style","")
	    $("#editorParseTreeContainer").hide()
	}

	nav.showDebugMap = function(delay) {
	    if (nav.debugMapRefreshTimer)
		window.clearTimeout (nav.debugMapRefreshTimer)
	    nav.debugMapRefreshTimer = window.setTimeout (function() {
		if (typeof(editor) == 'undefined')
		    nav.createEditor()
		editor.showDebugMap()
		delete nav.debugMapRefreshTimer
	    }, delay)
	}
 ...
 return $("#playerName")[0].value }
	...
	nav.Message.prototype.populatePlayerList = function() {
	    ...
	    for (var i = 0; i < n; ++i) { $("#playerList").append($('<li>').append(newCursor())) }
	}
...
//	    console.log ("Adding moveListener[" + type + "] on " + channel)
	    return moveListener[type] = client.subscribe(channel,callback)
	}

	function addPublisher(type,callback,interval) {
	    return publisher[type] = window.setInterval (callback, interval)
	}
	...
	function activateInvitationPublisher(msg) {
	    ...
	    addPublisher ('invitation', publishInvitation, invitationDelay)
	    publishInvitation()
	}
	...
	function activateApplicationListener(msg) {
		...
		if (message.gotAllPlayers()) {
		    invitationPublisher(message) ()  // publish complete player list: signals that invitation is canceled (HACK: would probably be cleaner to have a separate 'uninvite' channel, to be called when quitting from "Start game")
	...
	$(function(){
	    ...
	    nav.loadInitialGrammar()
	    if (editorArgs.startWithIDEVisible)
		$("#showIDECheckbox")[0].checked = true
	    nav.changeShowIDE()
	});
    }
    ...
    lw.Grammar.prototype.getIndexedNontermName = function(prefix,role) {
	var indexedName = prefix + (role + 1)
	var name = (indexedName in this.nonterm)
	    ? indexedName
	    : ((prefix in this.nonterm)
	       ? prefix
	       :  '')
	return name
    }
    ...
    function makeRhsExpr(props,rules,normalize,newId) {
	var str = makePreamblePlaceholderPrompt (props,normalize,newId);
	str += "{";
	for (var j = 0; j < rules.length; ++j) {
	    var rule = rules[j];
	    if (j > 0) str += "|";
	    var hint_with_modifiers = rule.hint + makeMaxUsage(rule) + makeTimesHidden(rule)
	    if (rule.weightExpr != null)
		str += rule.weightExpr
		+ (hint_with_modifiers.length ? ("=>" + hint_with_modifiers + "=>") : "==>")
	    else if (hint_with_modifiers.length)
		str += hint_with_modifiers + "=>"
	    str += rhsToString(rule.rhs,normalize,newId)
	}
	str += "}";
	return str;
    }

    function rhsToString(rhs,normalize,newId) {
	return rhs.map(function(sym){
	    if (sym instanceof LetterWriter.NontermReference) {
		var nonterm = sym.nonterminal;
		if (nonterm.anonymous) {
		    if (normalize)
			return makeNontermRef (sym.props, nonterm.id, normalize, newId);  // anonymous id remapping will have been set up as part of normalization
		    return makeRhsExpr (sym.props, nonterm.rules, normalize, newId);
		}
		return makeNontermRef (sym.props, nonterm.id, normalize, newId);
	    } else
		return sym.asText();
	}).join("");
    }

    function makeNontermRef(props,id,normalize,newId) {
	return makePreamblePlaceholderPrompt(props,normalize,newId)
	    + "@" + (((typeof(newId) != 'undefined') && id in newId) ? newId[id] : id)
	    + ((("pause" in props) && props.pause) ? ";" : "")
	    + ((("commit" in props) && props.commit) ? "!" : "")
	    + ((("random" in props) && props.random) ? "?" : "");
    }

    function makePreamblePlaceholderPrompt(props,normalize,newId) {
	var str = "";
	var gotPreamble = props.hasOwnProperty("preamble");
	var gotPostamble = props.hasOwnProperty("postamble");
	var gotPlaceholder = props.hasOwnProperty("placeholder");
	var gotPrompt = props.hasOwnProperty("prompt");
	if (gotPreamble || gotPlaceholder || gotPrompt || gotPostamble) {
	    str += "["
		+ (gotPreamble || gotPostamble
		   ? ((gotPreamble ? rhsToString(props.preamble,normalize,newId) : "")
		      + "|" + (gotPlaceholder ? props.placeholder : "")
		      + "|" + (gotPrompt ? props.prompt: "")
		      + (gotPostamble ? ("|" + rhsToString(props.postamble,normalize,newId)) : ""))
		   : ((gotPlaceholder
		       ? (props.placeholder + "|")
		       : "") + (gotPrompt ? props.prompt: "")))
		+ "]"
	...
	for (var i = 0; i < this.nonterms.length; ++i) {
		...
		str += "@" + id + makeMaxUsage(nonterm) + " => "
		    + makeRhsExpr (nonterm, nonterm.rules, normalize, newId) + "\n";
	    }
	}
	...
	addProperty (this, capitalize(wordForNonterminal)+" @", "id", {type:"text",size:15,keepFocus:false,hasWarning:false,parentDiv:propColumn[c++]}, "The word used to identify this "+wordForNonterminal+".", grammarEditor.nontermRenamingFilter(nonterm))
	...
	if (!grammar.isSinglePlayer()) {
		...
		addControl ({ property: "role",
			      parentDiv: propColumn[c++],
			      name: "Role",
			      inputDiv: $('<select/>').attr('name','nontermRole'+(++nteDummyCount)).append('<option value="=">Parent</option><option value="+">Next</option><option value="-">Previous</option>' + role.map(function(r){return'<option value="'+r+'">Role #'+r+'</option>'}).join('')),
			      hasWarning: false,
			      getInput: function(){return this.inputDiv.val()},
			      setInput: function(x){this.inputDiv.val(x)},
			      tooltip: "This control selects which role (i.e. which human or computer player in a multiplayer game) has control of this "+wordForNonterminal+". As well as specifying roles directly, you can specify that the controller is the same as for the parent symbol, or the previous player, or the next player.",
			      notify: function(){nontermEditor.updatePlayer()} });
	}

	addProperty (this, "Limit?", "maxUsage", {type:"text",size:3,parentDiv:propColumn[c++]}, "The Limit field specifies the maximum number of times that the "+wordForNonterminal+" " + nid + " may be used in the document. Leave this blank for no limit.");
	...
	function updateAdvancedView(reveal) {
		...
		nontermEditor.postambleControl.containerDiv.show()
		...
		if (!nonterm.hasOwnProperty("postamble"))
		    nontermEditor.postambleControl.containerDiv.hide()
	...
	var preambleInputDiv = $('<textarea rows="4"/>')
	function getPreamble() { return nonterm.preamble }
	function getPreambleAsText() { return nonterm.preambleAsText() }
	function setPreamble(newPreamble) {
	    nonterm.preamble = newPreamble;
	    preambleInputDiv.val (nonterm.preambleAsText()) }
	function getPreambleWarning() { return nontermEditor.preambleControl.warningDiv }
	var preambleInputChange = makeRhsInputChange (nontermEditor, getPreamble, setPreamble, function(){}, getPreambleWarning)

	this.preambleControl =
	    addControl ({property: "preamble",
			 inputDiv: preambleInputDiv,
			 keepFocus: false,
			 change: preambleInputChange,
			 get: getPreambleAsText,
			 tooltip: "This is the 'preamble' text. It is displayed wherever the "+wordForNonterminal+", " + nid + ", occurs in the narrative, and it goes before any other text or controllers associated with " + nid + ". It is not removed or changed after the player makes a choice, although it may be hidden from view."})
	...
	this.placeholderControl =
	    addControl ({property: "placeholder",
			 ...
			 keepFocus: false,
			 filter: validatePText(function(){return nontermEditor.placeholderControl.warningDiv},"The placeholder is not expanded or preprocessed at all, and so"),
			 tooltip: "This text is called the placeholder. It follows on from the preamble. The placeholder is visible only until the "+wordForNonterminal+", " + nid + ", is expanded; it disappears when the player (or computer) makes a choice. The texts flow seamlessly together; the player will not see any difference between the placeholder text and preceding text."})
	...
	this.promptControl =
	    addControl ({property: "prompt",
			 ...
			 inputDiv: $('<textarea rows="1"/>'),
			 keepFocus: false,
			 filter: validatePText(function(){return nontermEditor.promptControl.warningDiv},"The prompt is not expanded or preprocessed at all, and so"),
			 tooltip: "This is the question asked to the player whenever " + nid + " occurs in the narrative, prompting them to choose from the various hints, each of which leads to an expansion for " + nid + "."})
	...
	var deleteNontermLink = $('<a href="#">Delete</a>')
		...
		var incoming = nonterm.incomingNonterms(grammar,true);
		if (incoming.length) {
		    if (!confirm ("Really, delete "+nonterm.asText()+" from the "+wordForNonterminal+" list? This will not actually delete it but just erase its rules, since it is referenced elsewhere in the "+wordForNonterminal+" list ("+incoming.map(function(n){return n.asText()}).join(", ")+")"))
			return;
		    nontermEditor.resetNonterm()
		} else {
		    if (nonterm.isBare() ? false : !confirm ("Really, delete "+nonterm.asText()+" from the "+wordForNonterminal+" list?"))
			return;
	...
	if (grammarEditor.tooltips) {
	    deleteNontermLink.attr("title","Click to delete this "+wordForNonterminal+" from the "+wordForNonterminal+" list.").tooltip()
	    copyNontermLink.attr("title","Click to copy this "+wordForNonterminal+".").tooltip()
	}
	...
	var postambleInputDiv = $('<textarea rows="1"/>')
	function getPostamble() { return nonterm.postamble }
	function getPostambleAsText() { return nonterm.postambleAsText() }
	function setPostamble(newPostamble) {
	    nonterm.postamble = newPostamble;
	    postambleInputDiv.val (nonterm.postambleAsText()) }
	function getPostambleWarning() { return nontermEditor.postambleControl.warningDiv }
	var postambleInputChange = makeRhsInputChange (nontermEditor, getPostamble, setPostamble, function(){}, getPostambleWarning)

	this.postambleControl =
	    addControl ({property: "postamble",
			 name: "Exit",
			 inputDiv: postambleInputDiv,
			 keepFocus: false,
			 change: postambleInputChange,
			 get: getPostambleAsText,
			 tooltip: "This is the 'exit' text. It is displayed wherever the "+wordForNonterminal+", " + nid + ", occurs in the narrative, but it appears after any other text or controllers associated with " + nid + ". It is not removed or changed after the player makes a choice, although it may be revealed or hidden from view."})
	this.postambleDiv = this.postambleControl.inputDiv;
    ...
    function makeRhsInputChange (nontermEditor, getRhs, setRhs, warnEmptyRhs, getRhsWarningDiv) {
	var nonterm = nontermEditor.nonterm
	var grammarEditor = nontermEditor.grammarEditor
	var grammar = grammarEditor.grammar
	var notify = grammarEditor.notifyChange
	return function(e) {
	    var oldRhs = getRhs();
		...
		var newRhs
		var rhsWarningDiv = getRhsWarningDiv()
		if (newRhsText == "") {
		    warnEmptyRhs()
		    newRhs = []
		} else {
 		    try {
 			newRhs = LetterWriter.parser.parse (newRhsText, "rhs_expansion");
 			hideWarningIcon (rhsWarningDiv)
		    } catch (err) {
			var msg = buildErrorMessage(err);
			console.log("Syntax error:\n" + msg);
			showWarningIcon (rhsWarningDiv, "It looks like there might be a syntax error in this text. The error reported by the parser was as follows:\n" + msg)
	...
	({axis: "y",
	  update: function(event,ui) {
	      ...
	      nontermEditor.updateRuleIndex()
	      notify()
	    }});
    }

    lw.NontermEditor.prototype.updateRuleIndex = function() {
	var nonterm = this.nonterm;
	for (var i = 0; i < nonterm.rules.length; ++i)
	    nonterm.rules[i].ruleIndex = i
    }
...
//	setLocation()
	...
	if (random) {
		...
		this.ruleEditor[id].hintDiv.hide()
	    ...
	    for (var id in this.ruleEditor) {
		this.ruleEditor[id].hintDiv.show()
		if (this.grammarEditor.tooltips) {
    ...
    function validatePText(warningDivFunc,prefix) {
	return validateParsedText(warningDivFunc,prefix,"ptext")
    }

    function validatePlainText(warningDivFunc,prefix) {
	return validateParsedText(warningDivFunc,prefix,"text")
    }

    function validateParsedText(warningDivFunc,prefix,parse_symbol) {
	return function(text,oldText) {
		...
		try {
		    LetterWriter.parser.parse (text, parse_symbol);
		} catch (e) {
    ...
    lw.NontermEditor.prototype.addRuleDiv = function(rule,olderSiblingDiv) {
	...
	var deleteLinkDiv = $('<a href="#">Delete</a>')
	var copyLinkDiv = $('<a href="#">Duplicate</a>')
	var deleteDiv = $('<div/>').addClass("deleteRule").append(deleteLinkDiv).append(" / ").append(copyLinkDiv).append(" option")

	if (grammarEditor.tooltips) {
	    deleteLinkDiv.attr("title","Click to delete this option.").tooltip()
	    copyLinkDiv.attr("title","Click to duplicate this option.").tooltip()
	}
	...
	copyLinkDiv.click (function(e){
	    e.preventDefault();
	    var newRule = rule.clone()
	    newRule.id = nonterm.newRuleId()
	    var pos = indexOf (nonterm.rules, rule)
	    nonterm.rules.splice (pos + 1, 0, newRule)
	    nontermEditor.addRuleDiv (newRule, ruleDiv)
	    nontermEditor.updateRuleIndex()
	    nontermEditor.createRuleSortable()
	    nontermEditor.updatePlayer()

	    grammarEditor.refreshHeaders()
	    notify() })

	var getRhsAsText = function() { return rule.rhsAsText() }

	var weightInputDiv = $('<textarea rows="1"/>').attr({name: "weight" + rule.id})
	...
	function validateHint(random) {
	    ...
	    if (random)
		return validateAsWeight ("hint")
	    else  // !random
		validatePlainText(function(){return hintWarningDiv},"In a "+wordForNonterminal+" whose expansion is chosen by the player, the hint will not itself be expanded, and so")(rule.hint)
	}

	function validateWeight() {
	    validateAsWeight ("weightExpr")
	}

	function validateAsWeight(property) {
	    var hintWarningDiv = ruleEditor[property+"InputWarning"]
	    if (/[\S]/.test(rule[property])) {
		var errorPrefix = "For a "+wordForNonterminal+" whose expansion is chosen by the computer, the "+property+" should be a number (representing a probability or a relative rate) or the name of an attribute. This does not look like a number or the name of an attribute, at least not within this program's limited parsing abilities. The specific error message encountered was as follows: "
		var weightFunc;
		try {
		    weightFunc = LetterWriter.parser.parse (rule[property], "param_expr");
		} catch (e) {
		...
		if (bad.missing.length)
		    showWarningIcon (hintWarningDiv, "Some parameters in this expression (" + bad.missing.map(function(x){return"$"+x}).join(", ") + ") are not referenced anywhere in the "+wordForNonterminal+" list, so they are probably not going to work the way you intended.")
		else if (bad.unsafe.length)
		    showWarningIcon (hintWarningDiv, "Some parameters (" + bad.unsafe.map(function(x){return"$"+x}).join(", ") + ") are not assigned to sliders; they are only assigned as \"side-effects\" of other "+wordForNonterminal+" expansions. As such, I cannot (without a more elaborate analysis) guarantee that they will have a meaningful probabilistic value, or indeed any value at all, at the time this rule is expanded. I hope you know what you're doing.")
		else
	...
	function weightInputChange(e){
	    rule.weightExpr = e.target.value
	    if (rule.weightExpr.length == 0)
		rule.weightExpr = null
	    if ('weightFunc' in rule)
		delete rule.weightFunc
	    validateWeight()
	    nontermEditor.setHeader()
	    notify() }
	...
	function getRhs() { return rule.rhs }
	function setRhs(newRhs) { rule.rhs = newRhs; rhsInputDiv.val (rule.rhsAsText()) }
	function getRhsInputWarning() { return ruleEditor.rhsInputWarning }
	var rhsInputChange = makeRhsInputChange (nontermEditor, getRhs, setRhs, warnEmptyRhs, getRhsInputWarning)
	...
	var ruleEditor
		...
		showAdvanced: function(){this.controlInfo.maxUsage.containerDiv.show();
					 ...
					 this.controlInfo.weightExpr.containerDiv.show();
					 if (!nonterm.random)
					     this.controlInfo.hint.containerDiv.show(); },
		hideAdvanced: function() {
		    ...
		    if (nonterm.random) {
			if (!rule.hasOwnProperty("hint") || rule.hint.length == 0)
			    this.controlInfo.hint.containerDiv.hide();
		    } else {
			if (!rule.hasOwnProperty("weightExpr") || rule.weightExpr == null || rule.weightExpr.length == 0)
			    this.controlInfo.weightExpr.containerDiv.hide();
		    } } }
	...
	addRuleControl ({property: "weightExpr",
			 name: "Probability ",
			 tooltip: "This is a logical test or probability for whether the hint will be shown.",
			 inputDiv: weightInputDiv,
			 keepFocus: false,
			 change: weightInputChange})
	...
	if (typeof(olderSiblingDiv) == 'undefined')
	    ...
	    ruleDiv.insertAfter (olderSiblingDiv)
	...
	if (mapDiv.length) {
	    this.addNotify (function() { showDebugMap (refreshDelay) })
	    showDebugMap()
	}
	...
	$("#addSliderLink")
		...
		notify() })
	    .attr("title","Click to add a new slider-controllable attribute. This by itself will not make the attribute do anything; you also need to mark some "+plural(wordForNonterminal)+" as 'random', and then enter the name of the attribute in the 'Option (computer)' field for some of that "+wordForNonterminal+"'s expansions. This will signal to the computer that it should use the named attribute to control those random expansions.").tooltip()
	...
	return function(newId) {
		...
		if (newId in grammar.nonterm) {
		    alert ("You cannot change the name of this "+wordForNonterminal+" from @" + oldId + " to @" + newId + ", because there is already a "+wordForNonterminal+" called @" + newId + ".");
    ...
    // Rule
    lw.Rule = function(id,weight,hint,lhs,rhs,mods) {
	extend (this, { id: id,
			lhs: lhs,
			rhs: rhs })
	...
	var rule = new lw.Rule (this.id, this.weightExpr, this.hint, this.lhs, [], this, {})
    ...
    // static empty rule
    var emptyRule = new lw.Rule("-empty",1,"",null,[],{})
    emptyRule.ruleIndex = null
    ...
    function rhsAsText(rhs) {
	return rhs.map(function(sym){return sym.asText()}).join("")
    }

    lw.Rule.prototype.rhsAsText = function() {
	return rhsAsText (this.rhs)
    }
	...
	if (letter.ruleActive(this)) {
	    weight = 1
	    var expr = (this.weightExpr==null) ? "" : this.weightExpr
	    if (expr != "") {
		var weightFunc
		if (!('weightFunc' in this))
		    this.weightFunc = LetterWriter.ParamFunc.newFromString (expr)
		weightFunc = this.weightFunc
		if (typeof weightFunc != 'undefined') {
		    var weightEval = weightFunc.evaluate(scope,{letter:letter})
		    if (typeof(weightEval) != 'undefined')
			weight = weightEval > 0 ? weightEval : 0
		}
	    }
	}
	return weight
	...
	for (var rhsPos = 0; rhsPos < this.rhs.length; ++rhsPos) {
	    ...
	    // hackily convert NontermReference into NontermParent,
	    //  which will then be automagically expanded to NontermPreamble, NontermExpansion and NontermPostamble
	    // also, convert IndexedNontermReference into NontermParent or Term
	    if (sym instanceof lw.NontermReference || sym instanceof lw.IndexedNontermReference)
		sym = sym.substitutionSymbol(letter.grammar,parentNode)
...
//				   console.log("Paused at " + symNode.nodeIdentifier())
	    ...
	    var symIsAutoExpandable = (sym instanceof lw.NontermReference && sym.isAutoExpandable)
	    var symIsComputerControlled = (sym instanceof lw.NontermExpansion && symNode.isLocallyControlledByComputer())
	    if (symIsAutoExpandable || symIsComputerControlled) {
		symSpan.appendChild (symNode.placeholderSpan);
		var delayedExpand = (function(sym,symNode,symSpan,symIsAutoExpandable) {
		    return function() {
//			console.log("delayedExpand of " + symNode.nodeIdentifier())
			var rule = sym.randomRule (letter, symNode.getScope(), symNode)
			if (rule instanceof lw.Rule) {
			    rule.expand (symSpan, symNode.placeholderSpan, symNode)
			    if (!symIsAutoExpandable)
				symNode.publishWithStyle()
			}
		    }}) (sym, symNode, symSpan, symIsAutoExpandable)
		if (currentlyPaused()) {
...
//		    console.log("Pushing delayedExpand of " + symNode.nodeIdentifier())
	    ...
	    // set up expand/collapse notify chains
...
//		console.log("Showing " + symNode.nodeIdentifier())
	...
	// replace the placeholder, if it was attached
	if (oldSpan.parentNode == parentSpan)
	...
	if (parentNode.symbol.commit()) {
	    var node = parentNode
	    var gran
	    while (typeof(gran = node.parent) != 'undefined') {
		for (var sib = 0; sib < node.siblingIndex; ++sib) {
	...
	case "$#":  // role-indexed parameter ($#score means $score[1], $score[2], $score[3]...depending on role)
	case "$+":  // indexed by ((role+1) mod (total roles))
	case "$-":  // indexed by ((role-1) mod (total roles))
	case "#":  // value (a numeric constant)
	case "'":  // value (a string constant)
	case "x":  // evaluates l as grammar-expandable text
	case "?":  // conditional if-then-else
	    break;
	default:
    ...
    lw.ParamFunc.prototype.evaluate = function(scope,args) {
	var param;
	switch (this.op) {
	case "*": return this.l.evaluate(scope,args) * this.r.evaluate(scope,args);
	case "/": return this.l.evaluate(scope,args) / this.r.evaluate(scope,args);
	case "+": return this.l.evaluate(scope,args) + this.r.evaluate(scope,args);
	case "-": return this.l.evaluate(scope,args) - this.r.evaluate(scope,args);
	case ".": return String(this.l.evaluate(scope,args)) + String(this.r.evaluate(scope,args));
	case "!": return 1 - this.l.evaluate(scope,args);
	case "<": return (this.l.evaluate(scope,args) < this.r.evaluate(scope,args)) ? 1 : 0;
	case ">": return (this.l.evaluate(scope,args) > this.r.evaluate(scope,args)) ? 1 : 0;
	case "<=": return (this.l.evaluate(scope,args) <= this.r.evaluate(scope,args)) ? 1 : 0;
	case ">=": return (this.l.evaluate(scope,args) >= this.r.evaluate(scope,args)) ? 1 : 0;
	case "==": return (this.l.evaluate(scope,args) == this.r.evaluate(scope,args)) ? 1 : 0;
	...
	case "$-":
	    ...
	    var owner = scope.paramOwner(param)
	    return (param in owner.paramValue) ? owner.paramValue[param] : 0
	case "#": return this.value;
	case "'": return this.value;
	case "x": return ("letter" in args) ? args.letter.expandText(scope,this.l.evaluate(scope,args)) : "";
	case "?":  return (Math.random() <= (this.cond.evaluate(scope,args)) ? this.t : this.f)
		.evaluate(scope,args)
	default: return null
	}
    }
	...
	switch (this.op) {
	case "*":
	    return "&mul(" + this.l.asText() + "," + this.r.asText() + ")"
	case "/":
	    return "&div(" + this.l.asText() + "," + this.r.asText() + ")"
	case "+":
	    return "&add(" + this.l.asText() + "," + this.r.asText() + ")"
	case "-":
	    return "&sub(" + this.l.asText() + "," + this.r.asText() + ")"
	case ".":
	    return "&cat(" + this.l.asText() + "," + this.r.asText() + ")"
	case "<":
	    return "&lt(" + this.l.asText() + "," + this.r.asText() + ")"
	case ">":
	    return "&gt(" + this.l.asText() + "," + this.r.asText() + ")"
	case "<=":
	    return "&leq(" + this.l.asText() + "," + this.r.asText() + ")"
	case ">=":
	    return "&geq(" + this.l.asText() + "," + this.r.asText() + ")"
	case "==":
	    return "&eq(" + this.l.asText() + "," + this.r.asText() + ")"
	case "!=":
	    return "&neq(" + this.l.asText() + "," + this.r.asText() + ")"
	case "!":
	    return "&not(" + this.l.asText() + ")"
	case "?":
	    return "&if(" + this.cond.asText() + "," + this.t.asText() + "," + this.f.asText() + ")"
	case "x":
	    return "&expand(" + this.l.asText() + ")"
	case "$":
	case "$#":
	case "$+":
	case "$-":
	...
	case "?":
	    return setUnion (this.cond.getParams(), setUnion (this.t.getParams(), this.f.getParams()))
	case "$":
	case "$#":
	case "$+":
	case "$-":
	...
	case "?":
	    return this.cond.hasParam(id) || this.t.hasParam(id) || this.f.hasParam(id)
	case "$":
	case "$#":
	case "$+":
	case "$-":
	...
	case "$-":
	    ...
	    break;
	case "?":
	    this.cond.replace(oldParam,newParam);
	    this.t.replace(oldParam,newParam);
	    this.f.replace(oldParam,newParam);
	    break;
	case "#":
	case "'":
	default:
    ...
    lw.Scope = function(parent,role,roles) {
	extend (this, { parent:parent,
			paramValue:{},
			role: role,
			roles: roles })
    }
    lw.Scope.prototype.complete = false
    ...
    // ancestralScope is the oldest Scope that needs to be cloned, or leave undefined for full deep-clone
    lw.Scope.prototype.deepClone = function(ancestralScope) {
	var scope = this
	...
	if (type == '$#')
	    ...
	    p += "[" + (1 + ((this.role + 1) % this.roles)) + "]"
	else if (type == '$-')
	    p += "[" + (1 + ((this.role + this.roles - 1) % this.roles)) + "]"
	return p
    }

    lw.Scope.prototype.paramOwner = function(paramKey) {
	var scope = this;
	while (typeof(scope.parent) != 'undefined' && !(paramKey in scope.paramValue))
	    scope = scope.parent
	return scope
    }

    lw.Scope.prototype.isComplete = function() {
	var scope = this
	return scope.complete && (scope.parent == null || scope.parent.isComplete())
    }

    lw.Scope.prototype.startNewScopeThenUpdate = function(node) {
	var scope = this
	var inner = new lw.Scope (scope, node.role, scope.roles)
	if (scope.hasOwnProperty("complete"))
	    inner.complete = scope.complete
	inner.role = node.role
	node.symbol.updateScope(inner,args)
	return inner
    }

    lw.Scope.prototype.deepCloneAtPointOfDivergenceThenUpdate = function(node) {
	var scope = this
	var divergentScope = scope.paramOwner (node.symbol.makeParamKey (scope))
	var newScope = scope.deepClone (divergentScope)
	if (scope.hasOwnProperty("complete"))
	    newScope.complete = scope.complete
	newScope.role = node.role
	node.symbol.updateScope (newScope)
	return newScope
    }

    lw.Scope.prototype.cloneThenUpdate = function(node) {
	var scope = this
    ...
    lw.Scope.prototype.scopeWithRole = function(role) {
	var scope
	if (role == this.role)
	    scope = this
	else {
    ...
    lw.Nonterm.prototype.prompt = lw.Nonterm.prototype.defaultPrompt = "Your choice?"
    lw.Nonterm.prototype.placeholder = ""
    lw.Nonterm.prototype.preamble = []
    lw.Nonterm.prototype.postamble = []
    ...
    lw.Nonterm.prototype.addRule = function(weight,hint,rhs,mods) {
	var ruleIndex = this.rules.length
	this.rules.push(new lw.Rule(this.newRuleId(),weight,hint,this,rhs,mods))
    ...
    lw.Nonterm.prototype.preambleRuleId = function() { return this.id + "-preamble" }
    lw.Nonterm.prototype.postambleRuleId = function() { return this.id + "-postamble" }
    lw.Nonterm.prototype.parentRuleId = function() { return this.id + "-parent" }
    ...
    lw.Nonterm.prototype.preambleAsText = function() {
	return rhsAsText (this.preamble)
    }

    lw.Nonterm.prototype.postambleAsText = function() {
	return rhsAsText (this.postamble)
    }
	...
	if (this.hasActiveRules (letter)) {
	    if (this.isHyperlink() && /\S/.test(prompt)) {
		...
		var activeRules = [], activeRuleIndex = [], activeRuleWeight = []
		for (var i = 0; i < this.rules.length; ++i) {
		    var rule = this.rules[i]
		    if (letter.ruleActive (rule)) {
			var weight = rule.weight(letter,scope)
			if (weight > 0) {
			    activeRules.push (rule)
			    activeRuleIndex.push (activeRuleIndex.length)
			    activeRuleWeight.push (weight)
			}
		    }
		}

		activeRules = activeRuleIndex.sort(function(a,b){
		    return activeRuleWeight[b] - activeRuleWeight[a]
		}).map(function(i){
		...
		for (var i = 0; i < activeRules.length; ++i) {
		    ...
		    var hintText = rule.hint.length
			? letter.expandText(scope,rule.hint)
			: rule.rhs.map(function(sym){return sym instanceof LetterWriter.Term ? sym.text : ""}).join(" ")
...
//		    var i = select.selectedIndex
		continueButton.onclick = function() {
		    ...
		    formContainerDiv.className += " fade"
		    continueButton.className += " fade"
...
//		span.appendChild (select);
    ...
    lw.Nonterm.prototype.randomRule = function(letter,scope,node) {
	// node is not used in this function
	var total = 0;
	var weight = [];
	for (var i = 0; i < this.rules.length; ++i) {
    ...
    // calculate role
    lw.Nonterm.prototype.computeRole = function(grammar,parentRole,symbolRole) {
	if (typeof(symbolRole) == 'undefined')
	    symbolRole = this.role
	var totalRoles = grammar.roles
	var role = null
	switch (symbolRole) {
	case "+": role = (parentRole + 1) % totalRoles; break;
	case "-": role = (parentRole + totalRoles - 1) % totalRoles; break;
	case "=": role = parentRole; break;
	default: role = parseInt(symbolRole) - 1; break;
	}
	return role
    }
    ...
    function propAccessor(name) {
	return function() {
	    return (name in this.props) ? this.props[name] : this.nonterminal[name]
	}
    }

    extend (lw.NontermReference.prototype,
	    { preamble: propAccessor("preamble"),
	      postamble: propAccessor("postamble"),
	      ...
	      role: propAccessor("role") })

    lw.NontermReference.prototype.asText = function() {
	return makePreamblePlaceholderPrompt(this.props)
	    + this.nonterminal.asText()
	    + (("pause" in this.props) && this.props.pause ? ";" : "")
	    + (("commit" in this.props) && this.props.commit ? "!" : "")
	    + (("random" in this.props) && this.props.random ? "?" : "")
    }

    lw.NontermReference.prototype.expandedPlaceholder = function(letter,scope) {
	return letter.expandText (scope, this.placeholder())
    }

    lw.NontermReference.prototype.expandedPrompt = function(letter,scope) {
	return letter.expandText (scope, this.prompt())
    }

    lw.NontermReference.prototype.sanitizeQualifiers = function() {
	if (this.random()) {
	    if (this.commit()) {
		console.log ("Can't commit at randomized "+wordForNonterminal+" (@" + this.nonterminal.id + "). Ignoring '!' modifier")
    ...
    lw.NontermReference.prototype.randomRule = function(letter,scope,node) {
	return this.nonterminal.randomRule (letter, scope, node)
    }

    lw.NontermReference.prototype.computeRole = function(grammar,parentRole) {
	return this.nonterminal.computeRole (grammar, parentRole, this.role())
    }

    lw.NontermReference.prototype.substitutionSymbol = function(grammar,parentNode) {
	return new lw.NontermParent (this)
    }

    // NontermParent
    lw.NontermParent = function(ref) { extend (this, ref) }
    lw.NontermParent.prototype = new lw.NontermReference()
    lw.NontermParent.prototype.constructor = lw.NontermParent
    lw.NontermParent.prototype.debugLabel = function() { return this.nonterminal.asText() }
    lw.NontermParent.prototype.substitutionSymbol = function(grammar,parentNode) { return this }
    lw.NontermParent.prototype.isAutoExpandable = true
    lw.NontermParent.prototype.commit = function() { return false }
    lw.NontermParent.prototype.random = function() { return true }
    lw.NontermParent.prototype.randomRule = function(letter,scope,node) {
	if (typeof(node) != 'undefined' && node.atMaxRecursionDepth())
	    return emptyRule
	var rhs = [new lw.NontermPreamble(this), new lw.NontermExpansion(this), new lw.NontermPostamble(this)]
	return new lw.Rule(this.nonterminal.parentRuleId(),1,"",null,rhs,{})
    }

    // NontermPreamble
    lw.NontermPreamble = function(ref) { extend (this, ref) }
    lw.NontermPreamble.prototype = new lw.NontermReference()
    lw.NontermPreamble.prototype.constructor = lw.NontermPreamble
    lw.NontermPreamble.prototype.debugLabel = function() { return "preamble" }
    lw.NontermPreamble.prototype.substitutionSymbol = function(grammar,parentNode) { return this }
    lw.NontermPreamble.prototype.isAutoExpandable = true
    lw.NontermPreamble.prototype.role = function() { return "=" }
    lw.NontermPreamble.prototype.pause = function() { return true }
    lw.NontermPreamble.prototype.commit = function() { return false }
    lw.NontermPreamble.prototype.random = function() { return true }
    lw.NontermPreamble.prototype.randomRule = function() {
	return new lw.Rule(this.nonterminal.preambleRuleId(),1,"",null,this.nonterminal.preamble,{})
    }

    // NontermExpansion
    lw.NontermExpansion = function(ref) { extend (this, ref) }
    lw.NontermExpansion.prototype = new lw.NontermReference()
    lw.NontermExpansion.prototype.constructor = lw.NontermExpansion
    lw.NontermExpansion.prototype.debugLabel = function() { return "choice" }
    lw.NontermExpansion.prototype.substitutionSymbol = function(grammar,parentNode) { return this }
    lw.NontermExpansion.prototype.isAutoExpandable = false
    lw.NontermExpansion.prototype.role = function() { return "=" }
    lw.NontermExpansion.prototype.pause = function() { return true }

    // NontermPostamble
    lw.NontermPostamble = function(ref) { extend (this, ref) }
    lw.NontermPostamble.prototype = new lw.NontermReference()
    lw.NontermPostamble.prototype.constructor = lw.NontermPostamble
    lw.NontermPostamble.prototype.debugLabel = function() { return "exit" }
    lw.NontermPostamble.prototype.substitutionSymbol = function(grammar,parentNode) { return this }
    lw.NontermPostamble.prototype.isAutoExpandable = true
    lw.NontermPostamble.prototype.role = function() { return "=" }
    lw.NontermPostamble.prototype.commit = function() { return false }
    lw.NontermPostamble.prototype.random = function() { return true }
    lw.NontermPostamble.prototype.randomRule = function() {
	return new lw.Rule(this.nonterminal.postambleRuleId(),1,"",null,this.nonterminal.postamble,{})
    }

    // IndexedNontermReference
    lw.IndexedNontermReference = function(prefix) {
	extend (this, { prefix: prefix });
    }

    lw.IndexedNontermReference.prototype.clone = function() {
	var ref = new lw.IndexedNontermReference (this.prefix)
	return ref
    }

    lw.IndexedNontermReference.prototype.asText = function() {
	return '@#' + this.prefix
    }

    lw.IndexedNontermReference.prototype.substitutionSymbol = function(grammar,parentNode) {
	var name = grammar.getIndexedNontermName(this.prefix,parentNode.role)
	if (name != '') {
	    var nonterm = new lw.NontermReference (grammar.nonterm[name])
	    return new lw.NontermParent (nonterm)
	}
	return new lw.Term('')
    }
    ...
    // ParamAssignment
    lw.ParamAssignment = function(args) {
	extend (this, args)
    }
	...
	var lhsAsText = this.type + this.id
    ...
    lw.ParamAssignment.prototype.updateScope = function(scope,args) {
	...
	paramScope.paramValue[param] = this.value.evaluate(scope,args)
    }
	...
	return this.type + this.id + "=?" + (this.pause() ? ";" : "")
    }
	...
	if (grammar.hasScoreParam() && typeof(scoreContainerDiv) != 'undefined') {
	    ...
	    var wholeCellWidth = (100 / (scoreParams.length)) + '%'
//	    var quarterCellWidth = (25 / (scoreParams.length + .5)) + '%'
	    trDiv.setAttribute('style','width:'+wholeCellWidth)
	    tableDiv.appendChild(trDiv)
	    scoreContainerDiv.appendChild(tableDiv)

/*
	    function addQuarterCell() {
		var tdDiv = document.createElement('td')
		tdDiv.setAttribute ('style', 'width:'+quarterCellWidth)
...
//	    addQuarterCell()
		...
		scope.role = role;
		return scoreParamFunc.evaluate(scope,{letter:letter}) }
		...
		function updateScore() {
			...
			if (scoreDefined) {
				...
				originalDelta[r] = undefined  // guard against multiple invocations
		...
		function initialUpdateScore() {
		    if ("scoreTimer" in letter)
			clearTimeout (letter.scoreTimer)
		    updateScore()
		}
		if ("scoreRefreshTimer" in letter) {
		    clearTimeout (letter.scoreRefreshTimer)
		    delete letter.scoreRefreshTimer
		}
		letter.scoreRefreshTimer = window.setTimeout (initialUpdateScore, refreshDelay)
	    })
	}

	// OK, ready to go
	var startRef = new LetterWriter.NontermReference (grammar.start, {})
	var startParent = new LetterWriter.NontermParent (startRef)
	this.root = new LetterWriter.Node (this, startParent, undefined, undefined)
	parent.innerHTML = "";  // clear any loading animation
	parent.appendChild(this.root.span);

	startParent
	    .randomRule (this, null, this.root)  // generates @start.preamble@start.expansion@start.postamble
	    .expand (this.root.span, this.root.placeholderSpan, this.root)
	...
	function showDebugParseTree(delay) {
	    if (letter.debugParseTreeRefreshTimer)
		window.clearTimeout (letter.debugParseTreeRefreshTimer)
	    letter.debugParseTreeRefreshTimer = window.setTimeout (function(delay) {
		letter.debugParseTree = new lw.DebugParseTreeView (letter, parseDiv[0])
		delete letter.debugParseTreeRefreshTimer
	    }, delay)
	}
	if (parseDiv.length) {
	    this.addNotify (function() { showDebugParseTree (refreshDelay) })
	    showDebugParseTree()
	}
...
//	if (this.root.isLocallyControlled())
    ...
    lw.Letter.prototype.expandText = function(scope,text,visited) {
	var letter = this;
	if (typeof(visited) == 'undefined')
	    visited = {}
	// expand computer-controlled nonterminals and parameter assignments
	// for use within placeholders & prompts
	var nontermRe = /^(.*)(@#?)([A-Za-z_][0-9A-Za-z_]*)(.*)$/m
	var paramRe = /^(.*)\$([A-Za-z_][0-9A-Za-z_]*);(.*)$/m
	var arrayParamRe = /^(.*)(\$[#\+\-])([A-Za-z_][0-9A-Za-z_]*);(.*)$/m
	var match

	while (match = paramRe.exec(text)) {
	    var l = match[1], param = match[2], s = "", r = match[3]
	    var ref = new lw.ParamReference("$",param)
	    s = ref.evaluate(scope,{letter:letter})
	    text = l + s + r
	}

	while (match = arrayParamRe.exec(text)) {
	    var l = match[1], op = match[2], arrayParam = match[3], s = "", r = match[4]
	    var ref = new lw.ParamReference(op,arrayParam)
	    s = ref.evaluate(scope,{letter:letter})
	    text = l + s + r
	}

	while (match = nontermRe.exec(text)) {
	    var l = match[1], nontermType = match[2], nontermId = match[3], s = "", r = match[4]
	    if (nontermType == '@#')
		s = letter.grammar.getIndexedNontermName (nontermId, scope.role)
	    else if ((nontermId in letter.grammar.nonterm) && !(nontermId in visited)) {
		visited[nontermId] = 1
		var nonterm = letter.grammar.nonterm[nontermId]
		if (nonterm.random) {
		    var rule = nonterm.randomRule(letter,scope,undefined)
		    var parentRole = scope.role
		    scope.role = nonterm.computeRole (letter.grammar, parentRole)
		    var text = nonterm.preambleAsText() + rule.rhsAsText() + nonterm.postambleAsText()
		    s = letter.expandText (scope, text, visited)
		    scope.role = parentRole
		}
	    }
	    text = l + s + r
	}
	...
	if (symbol instanceof LetterWriter.Term)
	    ...
	    // compute role
	    if (parentNode instanceof LetterWriter.Node)
		this.role = symbol.computeRole (letter.grammar, parentNode.role)
	    else
		this.role = 0  // role #0 always owns root node

	    // there are four NontermReference nodes: NontermParent, NontermPreamble, NontermExpansion, NontermPostamble
	    // we only attach placeholder + controller/listener to NontermExpansion
	    if (symbol instanceof LetterWriter.NontermExpansion) {
		var scope = this.getScope()
		this.placeholderSpan.innerHTML = this.terminalTextToHtml(symbol.expandedPlaceholder(letter,scope));

		if (this.isLocallyControlledByHuman())
		    this.symbol.nonterminal.attach (this, this.span, this.placeholderSpan, symbol.expandedPrompt(letter,scope));
		...
		inputDiv.value = symbol.evaluate(this.getScope(),{letter:node.letter})
	    ...
	    var value = symbol.evaluate (this.getScope(),{letter:letter})
    ...
    lw.Node.prototype.isUnexpanded = function() { return this.expansionRule == null }
    ...
    lw.Node.prototype.atMaxRecursionDepth = function() {
	return this.recursionDepth() >= maxRecursionDepth
    }

    lw.Node.prototype.recursionDepth = function() {
	var n = this, nodeId = this.symbol.nonterminal.id, count = {}
	count[nodeId] = 0
	while (typeof(n) != 'undefined') {
	    if (!n.symbol.random())
		break  // don't count player choices as recursion
	    if ("parent" in n) {
		for (var olderSiblingIndex = 0; olderSiblingIndex < n.siblingIndex; ++n) {
		    var olderSibling = n.parent.child[olderSiblingIndex]
		    function needsHuman(node) {
			if (node.symbol instanceof lw.NontermExpansion
			    && !node.symbol.random())
			    return true
			for (var c = 0; c < node.child.length; ++c)
			    if (needsHuman (node.child[c]))
				return true
			return false
		    }
		    if (needsHuman (olderSibling))
			break  // don't count nodes that require player input as "recursion" (yes this is bending the definition of recursion... sorry)
		}
	    }
	    if (n.symbol instanceof lw.NontermParent) {
		var id = n.symbol.nonterminal.id
		if (!(id in count)) count[id] = 0
		++count[id]
	    }
	    n = n.parent
	}
	this.recursionDepth = function() { return count[nodeId] }
	return count[nodeId]
    }
...
//	    promise.then(function(){
		node.placeholderSpan.className = 'moveListener'
	        var waitText = document.createElement("SPAN")
		waitText.setAttribute ("class", "cursorText")
	        waitText.innerText = node.symbol.random()
		    ? "[Loading]"
		    : ("[Player" + (node.role + 1) + "]")
		node.placeholderSpan.appendChild (waitText)
		node.placeholderSpan.appendChild (newCursor())
//	    })
//	}
...
//		console.log("Newly-enabled option expanded at " + this.nodeIdentifier())
    ...
    lw.Node.prototype.hasParent = function() {
	return typeof(this.parent) != 'undefined'
    }

    lw.Node.prototype.hasOlderSibling = function() {
	return typeof(this.siblingIndex)!='undefined' && this.siblingIndex > 0
    }

    lw.Node.prototype.previousSibling = function() {
	return this.parent.child[this.siblingIndex - 1]
    }

    lw.Node.prototype.isOldestSiblingWithLocalParamAssignment = function() {
	function isLocalParamAssignment(n) {
	    return n.symbol instanceof lw.ParamAssignment && n.symbol.local
	}

	function hasNoOlderSiblingsWithLocalParamAssignment(n) {
	    if (!n.hasOlderSibling())
		return true
	    var s = n.olderSibling()
	    return !isLocalParamAssignment(s) && hasNoOlderSiblingsWithLocalParamAssignment(s)
	}

	return isLocalParamAssignment(this) && hasNoOlderSiblingsWithLocalParamAssignment(this)
    }

    lw.Node.prototype.isUnexpandedNontermReference = function() {
	var node = this
	return node.symbol instanceof lw.NontermReference
	    && (node.isUnexpanded() || node.unexpandedChildren > 0)
    }

    lw.Node.prototype.hasChildren = function() { return this.child.length > 0 }
    lw.Node.prototype.lastChild = function() { return this.child[this.child.length - 1] }

    lw.Node.prototype.getScopeInner = function(includingSelf) {
	var node = this
	// check cache
	var propName = includingSelf ? "postScope" : "preScope"
	if (propName in node)
	...
	if (!includingSelf) {
	    // pre
	    var pre
	    if (node.hasOlderSibling())
		pre = node.previousSibling().getScopeInner(true).scopeWithRole(node.role)
	    else if (node.hasParent())
		pre = node.parent.getScopeInner(false).scopeWithRole(node.role)
	    else {
		pre = new lw.Scope(undefined,node.role,node.letter.grammar.roles)
		pre.complete = true
	    }
	    if (pre.role != node.role)
		console.log("oopsy-pre")
	    scope = pre
	} else {
	    // post
	    var pre = this.getScopeInner(false)
	    var post = node.symbol instanceof lw.ParamAssignment
		? (node.isOldestSiblingWithLocalParamAssignment()
		   ? pre.startNewScopeThenUpdate(node)  // sets newScope.role = node.role
		   : pre.deepCloneAtPointOfDivergenceThenUpdate(node))
	    : (node.hasChildren()  // not instanceof lw.ParamAssignment
	       ? node.lastChild().getScopeInner(true).scopeWithRole(node.role)
	       : pre)

	    if (node.isUnexpandedNontermReference())
	    { post = post.clone(); post.complete = false }

	    if (post.role != node.role)
		console.log("oopsy-post")
	    scope = post
	}

	if (scope.isComplete() && !(propName in node))
	    node[propName] = scope
    ...
    lw.Node.prototype.debugSize = function() { return this.leaf?2:1 }
    lw.Node.prototype.debugLabel = function() { return this.symbol.debugLabel() }
    lw.Node.prototype.debugColor = function(letter) {
	return
	this.leaf
	    ? (this.symbol.nonterminal.numberOfActiveRules(letter) > 0
	       ? "#00cc00"
	       : "#ff0000")
	: (this.symbol.random()
	   ? "#ff88ff"
	...
	var node = this
	while (true) {
	    if (!(node.symbol instanceof lw.NontermPreamble
		  || node.symbol instanceof lw.NontermPostamble
		  || node.symbol instanceof lw.NontermExpansion))
		...
		break;
	    if (parent.symbol instanceof lw.NontermPreamble
		|| parent.symbol instanceof lw.NontermPostamble)
		suffices.unshift ('s' + (1 + node.siblingIndex))
	    else if (parent.symbol instanceof lw.NontermExpansion) {
		suffices.unshift ('s' + (1 + node.siblingIndex))
		suffices.unshift ('r' + (1 + node.sourceRule.ruleIndex))
	    } else if (parent.symbol instanceof lw.NontermParent)
		suffices.unshift (node.symbol instanceof lw.NontermPreamble
				  ? 'p'
				  : (node.symbol instanceof lw.NontermPostamble
				     ? 'q'
				     : 'x'))
	...
	function treeJSON(c) { return c.treeJSON() }
	var msgNode = null;
	if (this.symbol instanceof lw.NontermParent) {
	    msgNode = { name: this.symbol.nonterminal.id,
			type: "nonterm" }
	    if (!this.isUnexpanded()
		&& "ruleIndex" in this.expansionRule
		&& this.expansionRule.ruleIndex == null)  // handle unexpanded nodes due to reaching max recursion depth
		msgNode.rule = null
	    else if (this.child.length) {
		var preambleNode = this.child[0], expansionNode = this.child[1], postambleNode = this.child[2]
		msgNode.child = [ { type: "preamble" },
				  { type: "expansion" },
				  { type: "postamble" } ]
		if (!preambleNode.isUnexpanded())
		    msgNode.child[0].child = preambleNode.child.map (treeJSON)
		if (!expansionNode.isUnexpanded()) {
		    msgNode.child[1].rule = expansionNode.expansionRule.ruleIndex
		    msgNode.child[1].child = expansionNode.child.map (treeJSON)
		if (!postambleNode.isUnexpanded())
		    msgNode.child[2].child = postambleNode.child.map (treeJSON)
		}
	    }
	...
	var msgNodeExpanded = ('child' in msgNode);
	var nodeExpanded = (typeof(this.expansionRule) != 'undefined')
	var nodeAutoExpanded = this.symbol.isAutoExpandable
	if (msgNodeExpanded) {
	    if (nodeExpanded || nodeAutoExpanded) {
		if (nodeExpanded && this.expansionRule.ruleIndex != msgNode.rule) {
	...
	if ("client" in this.letter) {
	    ...
	    var publisherId = 'wait' + id
	    letter.addPublisher (publisherId,
				 function() {
				     console.log ("Republishing " + node.nodeIdentifier())
				     node.publish()
				 },
				 waitPublishInterval)
...
//	    this.letter.cancelMoveListener (id + '*')
	    this.letter.cancelPublisher ('wait' + id)
	}
    }
...
//	    console.log ("Publishing " + JSON.stringify(msg.tree) + " on " + this.nodeChannel())
	...
	function countDescendants(node) {
	    ...
	    return n }
	var debugParseNodes = nonterms + terms;
	...
	function makeNontermId(node,count,type) {
	    ...
	    return node.nodeIdentifier() + '/' + type + "/" + count
//	    return "@" + node.symbol.nonterminal.id + type + "/" + count
	...
	var getAttrs = function(node,attrs) {
	    var attr = {}
	    attr["Source text"] = quoteHtml(node.symbol.asText())
	    ...
	    for (var param in scope.paramValue)
		attr['$' + param] = scope.paramValue[param]
	    return extend(attr,attrs)
	}
	var addNode = function(node,parent,x) {
	    var attr = {Type:(node.symbol instanceof LetterWriter.NontermParent
			      ? "Parent"
			      : (node.symbol instanceof LetterWriter.NontermPreamble
				 ? "Preamble"
				 : (node.symbol instanceof LetterWriter.NontermExpansion
				    ? "Expansion"
				    : (node.symbol instanceof LetterWriter.NontermPostamble
				       ? "Postamble"
				       : "Unknown")))),
			Player:node.symbol.random()?"Computer":"Human",
			Role:node.role+1,
			Expanded:node.leaf?"not yet":"yes",
			"Recursion depth":node.recursionDepth(),
			stash:{node:node}}
	    var activeRules = 0
	    if (node.symbol instanceof LetterWriter.NontermExpansion) {
		if (node.leaf) {
	    ...
	    if (!((node.symbol instanceof LetterWriter.NontermPreamble
		   && node.symbol.nonterminal.preambleAsText() == '')
		  || (node.symbol instanceof LetterWriter.NontermPostamble
		      && node.symbol.nonterminal.postambleAsText() == '')))
		addDebugTreeNode(makeNontermId(node),makeNontermId(parent),extend({label:node.debugLabel(),x:x,color:node.debugColor(letter),size:node.debugSize()},getAttrs(node,attr))) }
	...
	var addCode = function(node,x) {
		...
		attr.stash.span = [node.span]
	    var evalsTo = isInput ? undefined : (isAssign ? node.symbol.value.evaluate(node.getScope(),{letter:node.letter}) : node.symbol.evaluate(node.getScope(),{letter:node.letter}))
